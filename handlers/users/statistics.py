...
@dp.callback_query_handler(text="Общая статистика")
async def callback_show_all_statistics(call: types.CallbackQuery):
...
async def back_to_all_statistics(call: types.CallbackQuery):
    ...
    await call.answer()
    try:
        await callback_show_all_statistics(call)
    except Exception: pass
...
# Перенос в профиль
@dp.callback_query_handler(text="Личная статистика")
async def show_mini_games_statistics(call: types.CallbackQuery):
    if not existe_in_db("mini_game_search_emoji_on_field", call.message.chat.id):
        add_mini_game_search_emoji_on_field_in_data_base(call.message.chat.id)
    add_last_message(call.message.chat.id)
    await call.answer()

    msg = "👤 <b><u>Личная статистика</u></b>"
    timee = count_time_in_bot(call.message.chat.id)
    if timee[0] == 0:
        if timee[1] == 0:
            msg += f"\n🕰 <b>Вы играете:</b> {timee[2]} ч. {0 if timee[3] < 0 else timee[3]} мин."
        else:
            msg += f"\n🕰 <b>Вы играете:</b> {timee[1]} дн. {timee[2]} ч. {timee[3]} мин."
    else:
        msg += f"\n🕰 <b>Вы играете:</b> {timee[0]} мес. {timee[1]} дн. {timee[2]} ч. {timee[3]} мин."
